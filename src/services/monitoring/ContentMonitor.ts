import { BlockedApp, ContentCategory, DetectionEvent, KeywordGroup, RiskLevel } from '@/types';
import { KeywordDetector, DetectionResult } from './KeywordDetector';

/**
 * Main monitoring orchestrator (singleton).
 * Handles both keyword detection and app-level blocking.
 */
export class ContentMonitor {
  ...
  /**
   * (Re-)initializes the detector with the given keyword groups and blocked apps.
   */
  initialize(keywordGroups: KeywordGroup[], blockedApps?: BlockedApp[]): void {
    ...
    return this.blockedApps.some(
      (app) =>
        ...
        (app.name.toLowerCase() === normalizedName ||
         app.packageName.toLowerCase() === normalizedName),
    );
  }

  /**
   * Gets the blocked app entry by name.
   */
  getBlockedApp(appName: string): BlockedApp | undefined {
    const normalizedName = appName.toLowerCase();
    return this.blockedApps.find(
      (app) =>
        app.isBlocked &&
        (app.name.toLowerCase() === normalizedName ||
         app.packageName.toLowerCase() === normalizedName),
   ...
   * Returns a DetectionEvent if keywords are found or the app is blocked, or null otherwise.
   */
  analyzeContent(text: string, appName: string): DetectionEvent | null {
    // Check if the app itself is blocked
    const blockedApp = this.getBlockedApp(appName);
    if (blockedApp) {
      return {
        id: this.generateId(),
        timestamp: new Date().toISOString(),
        appName,
        category: blockedApp.category,
        matchedKeywords: [`[앱 차단] ${blockedApp.name}`],
        riskScore: 100,
        action: 'blocked',
      };
    }
