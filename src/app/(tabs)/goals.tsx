...
export default function GoalsScreen() {
  ...
  const handleAddGoal = () => {
    ...
    // 목표를 크롬 확장 프로그램에 동기화 (차단 시 AI 영상 생성에 사용)
    const updatedGoals = [...(profile?.goals || []), { title: newTitle.trim(), description: newDesc.trim(), category: newCategory }];
    ChromeExtensionBridge.syncGoalsToExtension(
      updatedGoals.map(g => g.title),
      profile?.dreams || []
    );
    setNewTitle('');
    setNewDesc('');
